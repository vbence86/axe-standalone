...
const argv = yargs
	...
	.nargs('lerna-packages', 1)
	.describe('lerna-packages', 'Execute a11y against all lerna packages')
	.help('h')
	.version()
	.alias('h', 'help')
	.alias('v', 'version')
	.alias('l', 'lerna-packages')
	.alias('r', 'root')
...
function processReports(reports) {
	let sumViolations = 0;
	const violationsDetected = Object
		.keys(reports)
		.map(v => reports[v])
		.map(report => sumViolations = sumViolations + report.violations.length );

	if (sumViolations > 0) {
		log(chalk.red(`${sumViolations} accessibility violations have been detected in total.`));
...
if (argv['lerna-packages']) {
	
	const lernaPackagesPath = path.resolve(`${appDirectory}/packages/`);
	const reports = {};

	dl.list(`${lernaPackagesPath}/`, true, list => {
	  
	  const loop = list.reduce((promise, dir) => {
	  	return promise
	  		.then(() => exec({ indexHtml, serverPath: path.join(lernaPackagesPath, dir) }))
